<?php 
	if (isset($_GET['id']) && isset($_GET['quantite']))
	{
		...
		$sql = "SELECT * from panier where id_produit = :id AND id_user = :user";
		...
		{
			$sql = "UPDATE panier set quantite = quantite + :quantite where id_produit = :id_produit AND id_user = :id_user";
			$stmt = $pdo->prepare($sql);
			$stmt->bindParam(':quantite', $quantite);
